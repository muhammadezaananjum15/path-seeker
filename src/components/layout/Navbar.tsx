...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
        ...
        <div className="pointer-events-auto w-full max-w-[1280px] 2xl:max-w-[1440px] flex items-center justify-between backdrop-blur-xl rounded-full px-3 sm:px-5 py-2 shadow-xl border min-h-[58px] sm:h-[62px] transition-colors duration-300 bg-white/95 text-slate-900 border-slate-200">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            ...
            {isAuthenticated ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-2 sm:pl-2 sm:pr-3 py-1.5 rounded-full transition-all border text-xs font-bold bg-slate-100 hover:bg-slate-200 border-slate-200 text-slate-800"
                >
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-60 rounded-2xl border shadow-2xl py-2 z-50 p-1.5 space-y-1 bg-white border-slate-200 text-slate-800">
                    ...
                    {[
                      ...
                      <Link
                        ...
                        className="block px-3 py-2 rounded-xl text-xs font-semibold transition-colors hover:bg-slate-100"
                      >
                        {item.label}
                      </Link>
                    ))}
                    {user?.role === 'admin' && (
                      <Link
                        to="/admin"
                        onClick={() => setUserDropdownOpen(false)}
                        className="block px-3 py-2 rounded-xl text-xs font-bold text-purple-600 transition-colors hover:bg-purple-50"
                      >
                        Admin Suite Panel
                      </Link>
                    )}
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-rose-600 transition-colors hover:bg-rose-50"
                    >
                      Log Out
                    </button>
                  </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <Link
                  to="/login"
                  className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full font-bold text-[12px] transition-all whitespace-nowrap text-slate-700 hover:text-purple-600 hover:bg-slate-100"
                >
            ...
            <button
              ...
              className="lg:hidden p-1.5 sm:p-2 rounded-full shrink-0 ml-0.5 transition-colors text-slate-700 hover:bg-slate-100"
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="fixed inset-x-4 top-[85px] z-50 rounded-3xl p-5 shadow-2xl border space-y-2 lg:hidden animate-in fade-in slide-in-from-top-4 duration-200 bg-white text-slate-900 border-slate-200">
          ...
          {!isAuthenticated && (
            <div className="grid grid-cols-2 gap-2 pt-1">
              <Link
                to="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-2.5 rounded-xl border font-bold text-xs transition-colors border-slate-300 text-slate-800 hover:bg-slate-100"
              >
                Log In
              </Link>
              <Link
